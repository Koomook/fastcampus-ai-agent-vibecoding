...
class ClaudeService:
    ...
    def _extract_mcp_content(self, tool_result: dict[str, Any]) -> str:
        """
        ...
        This method extracts the actual text content and returns it as a string.

        Args:
            tool_result: Raw MCP tool result

        Returns:
            Extracted text content (usually JSON string from Notion API)
        """
        try:
                ...
                if response.stop_reason == "end_turn":
                    ...
                    for content_block in response.content:
                        if content_block.type == "tool_use":
                            ...
                            logger.info(f"Tool result preview: {str(tool_result)[:500]}")

                            # Extract text content from MCP response
                            # MCP returns: {'content': [{'type': 'text', 'text': '...JSON...'}]}
                            tool_result_content = self._extract_mcp_content(tool_result)
